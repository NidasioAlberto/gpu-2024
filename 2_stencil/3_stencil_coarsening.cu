...
__global__ void stencil_kernel_gpu(const float *__restrict__ input, float *__restrict__ output) {
    ...
    __shared__ float tile_previous[IN_TILE_DIM][IN_TILE_DIM];
    __shared__ float tile_current[IN_TILE_DIM][IN_TILE_DIM];
    __shared__ float tile_next[IN_TILE_DIM][IN_TILE_DIM];

    if (x < DATA_DIM && y < DATA_DIM && z - 1 < DATA_DIM) {
        tile_previous[threadIdx.y][threadIdx.x] = input[idx(x, y, z - 1, DATA_DIM)];
    }
    if (x < DATA_DIM && y < DATA_DIM && z < DATA_DIM) {
        tile_current[threadIdx.y][threadIdx.x] = input[idx(x, y, z, DATA_DIM)];
    }
    for (; z < (blockIdx.z + 1) * OUT_TILE_DIM; z++) {
        if (z + 1 < DATA_DIM && x < DATA_DIM && y < DATA_DIM) {
            tile_next[threadIdx.y][threadIdx.x] = input[idx(x, y, z + 1, DATA_DIM)];
        }
        __syncthreads();
        ...
        if (x >= 1 && x < DATA_DIM - 1 && y >= 1 && y < DATA_DIM - 1 && z >= 1 && z < DATA_DIM - 1) {
            ...
            if (threadIdx.x >= 1 && threadIdx.x < IN_TILE_DIM - 1 && threadIdx.y >= 1 &&
                ...
                float value = constant_coefficients.c0 * tile_current[threadIdx.y][threadIdx.x];
                value += constant_coefficients.c1 * tile_current[threadIdx.y][threadIdx.x - 1];
                value += constant_coefficients.c2 * tile_current[threadIdx.y][threadIdx.x + 1];
                value += constant_coefficients.c3 * tile_current[threadIdx.y - 1][threadIdx.x];
                value += constant_coefficients.c4 * tile_current[threadIdx.y + 1][threadIdx.x];
                value += constant_coefficients.c5 * tile_previous[threadIdx.y][threadIdx.x];
                value += constant_coefficients.c6 * tile_next[threadIdx.y][threadIdx.x];
        ...
        tile_previous[threadIdx.y][threadIdx.x] = tile_current[threadIdx.y][threadIdx.x];
        tile_current[threadIdx.y][threadIdx.x] = tile_next[threadIdx.y][threadIdx.x];
    }
}
