...
__global__ void convolution_constant(float *__restrict__ input, float *__restrict__ output) {
    // Data position
    ...
    for (int x_f = 0; x_f < FILTER_DIM; x_f++) {
        for (int y_f = 0; y_f < FILTER_DIM; y_f++) {
            ...
            if (x_cell_global >= 0 && x_cell_global < DATA_DIM && y_cell_global >= 0 && y_cell_global < DATA_DIM) {
                ...
                // One from input => 1 memory accesses => 4 bytes
                // 2 / 8 = 0.5

                // The constant memory does not show any improvement over the naive implementation
                // it even shows a slowdown!
                // It shows a lower memory throughput useb but an higher compute resources usage
            }
        }
    }
...
int main() {
    ...
    convolution_constant<<<blocks_per_grid, threads_per_block>>>(d_input, d_output);
    CHECK_KERNELCALL();
