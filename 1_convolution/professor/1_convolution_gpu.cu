...
__global__ void convolution_basic_kernel(const input_type *__restrict__ input, const filter_type *__restrict__ filter,
                                         input_type *__restrict__ output, const int width, const int height,
                                         const int filter_size, const int filter_radius) {
...
int main(int argc, char **argv) {
    ...
    CHECK(cudaMemcpy(d_filter, filter, FILTER_SIZE * FILTER_SIZE * sizeof(filter_type), cudaMemcpyHostToDevice));
    ...
    convolution_basic_kernel<<<numBlocks, threadsPerBlock>>>(d_input, d_filter, d_output, width, height, FILTER_SIZE,
                                                             FILTER_RADIUS);
    CHECK_KERNELCALL();
